...
void ObjModel::InitializeVertex(const std::string& filename)
{
	HRESULT result;
	//ファイルストリーム
	std::ifstream file;
...
void ObjModel::CompileShader(const wchar_t* file, const wchar_t* file2)
{
	...
	result = D3DCompileFromFile(
		L"Resources/Shaders/Obj/ObjVertexShader.hlsl",	//シェーダファイル名
		nullptr,
	...
	result = D3DCompileFromFile(
		L"Resources/Shaders/Obj/ObjPixelShader.hlsl",	//シェーダファイル名
		nullptr,
		D3D_COMPILE_STANDARD_FILE_INCLUDE,	//インクルード可能にする
		"main",		//エントリーポイント名
		"ps_5_0",	//シェーダモデル指定
...
void ObjModel::LoadTexture(const std::string& resourcename)
{
